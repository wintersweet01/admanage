        ...
        <form method="get" action="" class="form-inline">
            ...
            <div class="form-group">
                ...
                <select name="monitor_id" id="monitor_id">
                    ...
                    <{foreach from=$data._Monitor item=name}>
                <option value="<{$name.monitor_id}>" <{if $data.monitor_id==$name.monitor_id}>selected="selected"<{/if}>> <{$name.name}> </option>
                    <{/foreach}>
                </select>
                ...
                <table class="table table-bordered table-bordered">
                    ...
                    <{foreach from=$data.list key=key item=item}>
                        <tr>
                            ...
                            <td nowrap><a target="_blank" href="<{$smarty.const.CDN_URL}><{$item.page_url}>/index.html"><{$smarty.const.CDN_URL}><{$item.page_url}>/index.html</a></td>
...
<script>
    $(function(){
        ...
        $('#printExcel').click(function(){
            ...
            location.href='?ct=adDataIOS&ac=hourLandExcel&parent_id='+$('select[name=parent_id]').val()+'&game_id='+$('select[name=game_id]').val()+channel_ids+'&package_name='+$('select[name=package_name]').val()+'&sdate='+$('input[name=sdate]').val()+'&edate='+$('input[name=edate]').val();
        });
        ...
        $('select[name=game_id]').on('change',function(){
            ...
            $.getJSON('?ct=platform&ac=getMonitorByGame&game_id='+game_id,function(re) {
                var html = '<option value="">全部</option>';
                $.each(re,function(i,n){
                    html += '<option value="'+i+'">'+n+'</option>';
                });
                $('#monitor_id').html(html);
            });
        });
    });
