    ...
    <div class="rows" style="margin-bottom: 0.8%; overflow: hidden;">
        <form method="get" action="" class="form-inline">
            ...
            <div class="form-group form-group-sm" style="margin-bottom: 10px;">
                <label>流量平台：</label>
                <{foreach $_channels as $v}>
                <label class="checkbox-inline">
                    <input type="checkbox" name="channel_id[]" value="<{$v.channel_id}>" <{if in_array($v.channel_id, $data.channel_id)}>checked="checked"<{/if}>>
                    <img src="<{if $v.logo}>uploads/<{$v.logo}><{else}>static/images/default.png<{/if}>" class="img-thumbnail">
                    <{$v.channel_name}>
                </label>
                <{/foreach}>
            </div>
                ...
                <table class="table table-bordered table-hover table-condensed table-striped table-responsive">
                    ...
                    <{foreach from=$data.list item=u}>
                        <tr>
                            ...
                            <td nowrap><{$_channel_list[$u.channel_id]}></td>
                            ...
                            <td nowrap><{if $u.material_count > 0}><span class="glyphicon glyphicon-ok green" aria-hidden="true"></span><{else}><span class="glyphicon glyphicon-remove red" aria-hidden="true"></span><{/if}></td>
                            ...
                            <td nowrap class="cursor">
                                <a href="?ct=material&ac=bindMaterial&material_id=<{$u.material_id}>" class="btn btn-info btn-xs">绑定推广</a><br>
                                <{if SrvAuth::checkPublicAuth('edit',false)}><a href="?ct=material&ac=editMaterial&material_id=<{$u.material_id}>" class="btn btn-primary btn-xs">编辑素材</a><br><{/if}>
                            ...
                            <td nowrap>
                                <div class="input-group input-group-sm">
                                    <input type="text" class="form-control" placeholder="开始时间" name="test_start" data-id="<{$u.material_id}>" value="<{if $u.test_start}><{$u.test_start}><{/if}>">
                                ...
                                <div class="input-group input-group-sm">
                                    <input type="text" class="form-control" placeholder="结束时间" name="test_end" data-id="<{$u.material_id}>" value="<{if $u.test_end}><{$u.test_end}><{/if}>">
                                    <span class="input-group-btn">
                                        <button class="btn btn-primary btnTime" type="button">确定</button>
                                    </span>
                                </div>
