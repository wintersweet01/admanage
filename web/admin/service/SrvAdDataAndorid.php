...
class SrvAdDataAndorid
{
        ...
        if ($info['list']) {
            ...
            foreach ($info['list'] as $key => $val) {
                foreach ($val as $k => $v) {
                    ...
                    $info['total']['total_money1'] += $v['money1'] / 100;
            ...
            if ($info['total']['total_reg'] > 0) {
                ...
                $info['total']['new_pay_rate'] = number_format($info['total']['total_new_pay'] / $info['total']['total_reg'] * 100, 2, '.', '') . '%';
                $info['total']['new_ARPU'] = number_format($info['total']['total_new_pay_money'] / $info['total']['total_reg'], 2, '.', '');
                $info['total']['total_ltv0'] = number_format($info['total']['total_money1'] / $info['total']['total_reg'], 2, '.', '');
    ...
    public function activityEffect($parent_id, $game_id, $channel_id, $rsdate, $redate, $psdate, $pedate, $is_excel = 0)
    {
        ...
        $data = $total = [];
        $info = $this->mod->activityEffect($parent_id, $channel_id, $game_id, $rsdate, $redate, $psdate, $pedate, $is_excel);
        foreach ($info as &$row) {
            $row['reg'] = (int)$row['reg'];
            $row['retain'] = (int)$row['retain'];
            $row['display'] = (int)$row['display'];
            $row['click'] = (int)$row['click'];
            $row['pay_count'] = (int)$row['pay_count'];
            $row['cost'] /= 100;
            $row['total_money'] /= 100;
            $row['pay_money'] /= 100;

            $row['retain_cost'] = $row['retain'] > 0 ? bcdiv($row['cost'], $row['retain'], 2) : 0;
            $row['roi'] = $row['cost'] > 0 ? bcdiv($row['pay_money'] * 100, $row['cost'], 2) : 0;
            $row['arpu'] = $row['reg'] > 0 ? bcdiv($row['pay_money'], $row['reg'], 2) : 0;
            $row['pay_rate'] = $row['reg'] > 0 ? bcdiv($row['pay_count'] * 100, $row['reg'], 2) : 0;
            $row['reg_cost'] = $row['reg'] > 0 ? bcdiv($row['cost'], $row['reg'], 2) : 0;
            $row['retain_rate'] = $row['reg'] > 0 ? bcdiv($row['retain'] * 100, $row['reg'], 2) : 0;
            $row['pay_cost'] = $row['pay_count'] > 0 ? bcdiv($row['cost'], $row['pay_count'], 2) : 0;
            $row['arppu'] = $row['pay_count'] > 0 ? bcdiv($row['pay_money'], $row['pay_count'], 2) : 0;
        ...
        $total['retain_cost'] = $total['retain'] > 0 ? bcdiv($total['cost'], $total['retain'], 2) : 0;
        $total['roi'] = $total['cost'] > 0 ? bcdiv($total['pay_money'] * 100, $total['cost'], 2) : 0;
        $total['arpu'] = $total['reg'] > 0 ? bcdiv($total['pay_money'], $total['reg'], 2) : 0;
        $total['pay_rate'] = $total['reg'] > 0 ? bcdiv($total['pay_count'] * 100, $total['reg'], 2) : 0;
        $total['reg_cost'] = $total['reg'] > 0 ? bcdiv($total['cost'], $total['reg'], 2) : 0;
        $total['retain_rate'] = $total['reg'] > 0 ? bcdiv($total['retain'] * 100, $total['reg'], 2) : 0;
        $total['pay_cost'] = $total['pay_count'] > 0 ? bcdiv($total['cost'], $total['pay_count'], 2) : 0;
        $total['arppu'] = $total['pay_count'] > 0 ? bcdiv($total['pay_money'], $total['pay_count'], 2) : 0;
        ...
        LibUtil::clean_xss($psdate);
        LibUtil::clean_xss($pedate);
        $data['reg_sdate'] = $rsdate;
        $data['reg_edate'] = $redate;
        $data['pay_sdate'] = $psdate;
        $data['pay_edate'] = $pedate;
    ...
    public function dayChannelEffect($channel_id, $game_id, $sdate, $edate, $pay_sdate, $pay_edate, $monitor_id, $is_excel)
    {
    ...
    public function hourLand($page, $package_name, $parent_id, $game_id, $channel_id, $sdate, $edate, $all, $is_excel = 0)
    {
        ...
        $info = $this->mod->hourLand($page, $package_name, $parent_id, $game_id, $channel_id, $sdate, $edate, $all, $is_excel);
        if ($info['total']['c'] > 0) {
            foreach ($info['list'] as $key => $val) {
                if ($val['click']) {
