...
class LibFile
{
    ...
    public static function uploadToTools($name, $size = 1000, $exts = array(), $fileName = '')
    {
        ...
        //格式验证
        $ext = self::getExt($_FILES[$name]['name']);
        ...
        $files = array(
            'fileData' => array('path' => $_FILES[$name]['tmp_name'], 'type' => $_FILES[$name]['type'], 'name' => $_FILES[$name]['name']),
        );
    ...
    {
        $info = array();
        if ($uploadPath && substr($uploadPath, -1, 1) != '/' && substr($uploadPath, -1, 1) != '\\') {
            $uploadPath .= '/';
        }
        if ($uploadUrl && substr($uploadUrl, -1, 1) != '/') {
            $uploadUrl .= '/';
        }
        ...
        $info['uploadpath'] = $uploadPath . $_fileName;
        $info['uploadUrl'] = $uploadUrl . $_fileName;
    ...
    //遍历目录
    public static function fileListPro($basePath, $dirName = '', $orderBy = 'name', $orderType = 'asc')
    {//order by time size name  | desc asc
