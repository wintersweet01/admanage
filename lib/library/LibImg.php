...
*/
    public static function imagezoom($srcimage, $dstimage, $dst_width, $dst_height, $backgroundcolor, $quality = 100)
    {
        // 中文件名乱码
        if (PHP_OS == 'WINNT') {
        ...
        $color = imagecolorallocatealpha($dstimg, hexdec(substr($backgroundcolor, 1, 2)), hexdec(substr($backgroundcolor, 3, 2)), hexdec(substr($backgroundcolor, 5, 2)), 127);
        imagefill($dstimg, 0, 0, $color);
        if (!$arr = getimagesize($srcimage)) {
            return array('state' => false, 'msg' => '要生成缩略图的文件不存在');
        }

        $src_width = $arr[0];
        $src_height = $arr[1];
        $srcimg = null;
        $method = self::getcreatemethod($srcimage);
        ...
        if (!in_array($suffix, array_keys($arr))) {
            return array('state' => false, 'msg' => '保存的文件名错误');
        } else {
            if ($suffix == 'png') {
                $quality = (10 - ($quality / 10));
            }
            eval($arr[$suffix] . '($dstimg, "' . $dstimage . '",' . $quality . ');');
            Debug::log($arr[$suffix] . '($dstimg, "' . $dstimage . '",' . $quality . ');');
        }
    ...
    public static function imagezoomLa($srcimage, $dstimage, $dst_width, $dst_height, $backgroundcolor, $quality = 100)
    {
        // 中文件名乱码
        if (PHP_OS == 'WINNT') {
            $srcimage = iconv('UTF-8', 'GBK', $srcimage);
        ...
        imagecopyresampled($dstimg, $srcimg, 0, 0, 0, 0, $dst_w, $dst_h, $src_width, $src_height);
        imagesavealpha($dstimg, true);
        // 保存格式
        $arr = array(
            'jpg' => 'imagejpeg',
     ...
     * @return array
     */
    public static function imagecut($srcimage, $dstimage, $start_x, $start_y, $dst_width, $dst_height, $src_width, $src_height, $backgroundcolor, $quality = 100)
    {
        // 中文件名乱码
        if (PHP_OS == 'WINNT') {
            $srcimage = iconv('UTF-8', 'GBK', $srcimage);
        ...
        imagecopyresampled($dstimg, $srcimg, 0, 0, $start_x, $start_y, $dst_width, $dst_height, $src_width, $src_height);
        imagesavealpha($dstimg, true);
        // 保存格式
        $arr = array(
            'jpg' => 'imagejpeg',
    ...
    public static function getcreatemethod($file)
    {
        $arr = array(
...
// BMP 保存函数，php本身无
     ...
     * @return bool
     */
    public static function onColor($file, $saveFile, $hex)
    {
        $size = getimagesize($file);
        $img = imagecreatefrompng($file);
        $new_img = imagecreate($size[0], $size[1]);
        $bg = imagecolorallocate($new_img, 255, 255, 255);
        $trans = imagecolortransparent($new_img, $bg);
        $rgb = self::hex2Rgb($hex);
        $color = imagecolorallocate($new_img, $rgb['r'], $rgb['g'], $rgb['b']);

        for ($i = 0; $i < $size[0]; $i++) {
            for ($j = 0; $j < $size[1]; $j++) {
                $_rgb = imagecolorat($img, $i, $j);
                $rgb = array(
                    ($_rgb >> 16) & 0xFF,
                    ($_rgb >> 8) & 0xFF,
                    $_rgb & 0xFF,
                ...
                if ($rgb[0] == 0 && $rgb[1] == 0 && $rgb[2] == 0) {
                    ...
                    //上色区域
                    imagesetpixel($new_img, $i, $j, $color);
                }
            }
        }
        if (imagepng($new_img, $saveFile)) {
            return $saveFile;
        }
        return false;
    }
